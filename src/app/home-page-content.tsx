...
export default function HomePageContent() {
  ...
  // Si no hay usuario, crear usuario anónimo automáticamente y redirigir al tablero
  useEffect(() => {
    if (!user && !isUserLoading && !isLoggingIn && !loginDialogOpen && auth && !hasProcessedUserRef.current) {
      // Crear usuario anónimo automáticamente para que puedan usar el tablero
      const createTempUser = async () => {
        try {
          ...
          if (result?.user && firestore) {
            await ensureUserDocument(firestore, result.user);
            // El processUser se ejecutará automáticamente y creará/redirigirá al tablero
            if (processUserRef.current) {
              processUserRef.current(result.user);
            }
          }
  ...
  }, [user, isUserLoading, isLoggingIn, loginDialogOpen, auth, firestore]);

  // Si no hay usuario aún, mostrar loading mientras se crea el usuario anónimo
  ...
  // Si no hay usuario después de intentar crear uno, mostrar loading
  if (!user) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-gray-50">
        <Loader2 className="h-8 w-8 animate-spin text-blue-600" />
  ...
  // Si hay usuario, el useEffect ya maneja la redirección automática
  return null;
}
