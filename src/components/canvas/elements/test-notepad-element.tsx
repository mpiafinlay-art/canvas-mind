...
export default function TestNotepadElement(props: CommonElementProps) {
  const { 
    id,
    content,
    properties,
  ...
  const { saveStatus, handleBlur: handleAutoSaveBlur, handleChange, forceSave } = useAutoSave({
    ...
    onSave: async (newHtml) => {
      ...
      const currentPageContent = currentPages[currentPageIndex] || '';
    ...
    compareContent: (oldContent, newContent) => {
      const currentPages = typedContent.pages || [''];
      const currentPageContent = currentPages[currentPageIndex] || '';
      return oldContent === currentPageContent;
    },
  });
  ...
  useEffect(() => {
    if (isListening && isSelected && contentRef.current && document.activeElement === contentRef.current) {
  ...
  const handleExportNotepadToPng = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      ...
      canvas.toBlob((blob: Blob | null) => {
        ...
        link.href = url;
        const notepadTitle = typedContent.title || 'cuaderno-test';
      ...
      return (
          <Card className="notepad-card w-full h-full flex items-center shadow-lg rounded-lg bg-card border-2 border-primary/50 group" data-element-id={id}>
               <div className="p-2 flex flex-row items-center gap-1 w-full cursor-grab active:cursor-grabbing drag-handle">
                  ...
                  <p className="font-headline text-sm font-semibold truncate flex-grow">{typedContent.title || 'Cuaderno Test'}</p>
  ...
  return (
    <Card data-element-id={id} className="notepad-card w-full h-full flex flex-col shadow-lg rounded-lg bg-card group">
        <div className="p-2 border-b flex flex-row items-center gap-1 cursor-grab active:cursor-grabbing drag-handle">
            ...
            <div
                ...
                data-placeholder='Cuaderno Test'
        ...
        {!minimized && (
            <>
                <CardContent className="p-0 flex-grow relative bg-card font-body overflow-y-auto">
                    <div className={cn("notepad-content-container", isSmallFormat && "small-format")}>
                        <div className={cn("notepad-lines-background", isSmallFormat && "small-format")} />
                    {!isPreview && (
                       <div 
                      ...
                      <div className='absolute inset-0 bg-white/95 z-20 p-4 text-xs overflow-y-auto' onClick={() => setIsInfoOpen(false)}>
                        ...
                        <div
                            ...
                            className={cn(
                                ...
                                !isSmallFormat && "p-[32px_24px_16px_0px]",
                                isSmallFormat && "p-[16px_12px_8px_0px]"
                            )}
                        />
